...
class Auth extends EventEmitter {
  ...
  async loginWithCookies(cookies, options = {}, callback) {
    ...
    try {
      ...
      // Restore device fingerprint if session state object was provided
      let rawState = cookies;
      if (typeof cookies === 'string') {
        try { rawState = JSON.parse(cookies); } catch (_) {}
      }
      if (rawState && typeof rawState === 'object') {
        if (rawState.deviceId) this.http.deviceId = rawState.deviceId;
        if (rawState.phoneId) this.http.phoneId = rawState.phoneId;
        if (rawState.uuid) this.http.uuid = rawState.uuid;
        if (rawState.userAgent) this.http.userAgent = rawState.userAgent;
      }
